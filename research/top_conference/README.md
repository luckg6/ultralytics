...
- 第四章最新筛选显示：SGC 为 Strip R-CNN 启发的条带几何校准候选，单模块在 DIOR-R seed 42 上有效；直接 `SGC+FDF` 组合存在负交互，不继续扩展。`FDR-Lite` 作为更温和的频率细节残差 D 候选已完成 seed 42 筛选，但 `SGC+FDR-Lite` 仍未超过 SGC 单模块，因此也不作为最终 C/D。
