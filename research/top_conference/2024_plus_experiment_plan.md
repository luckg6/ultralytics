...
- C-GRA-Lite 已完成代码和配置，参考 ECCV 2024 GRA 的 group-wise rotating / attention 思想，用标准 PyTorch 的方向掩码 depthwise 分支做轻量适配，当前待训练。
...
- 风险：参数增幅高于 C-Dynamic-Plus；建议先训练单独 C-GRA-Lite，若明显优于 C-Plus，再训练 ABC-GRA-Lite。
...
补充：A+B+C-Plus 已完成但低于 A+B-PKI-Lite；C-GRA-Lite 已 ready，建议先训单点再训 ABC-GRA-Lite
备选：B-FreqFuse -> C-GRA-Lite 后续加强
