...
GauCho 针对 OBB 角度边界不连续问题，使用 Cholesky 分解直接预测二维高斯分布参数。本项目曾完成训练期 C-Chol-Lite 辅助方向探索，但未进入第三章定稿方法；完整 GauCho head 仍未迁移。
...
- 高风险的 head/loss 后备方向，更适合作为讨论或后续扩展，不是当前第四章执行路线。

## 风险

- 需要深入改 OBB head、loss 和后处理，影响面大。
- 与 Ultralytics 当前 OBB 表示耦合较深，调试成本高。
- 若未来采用，必须重新建立独立受控消融，不能把 C-Chol-Lite 当作完整 GauCho 复现。
