...
该 baseline 不继承第三章 FSPB/LPCF。LSKNet-T 只是基础架构选择，不作为创新。OAC/FDF、Blend、FDConv-Lite、直接 `SGC+FDF` 和 `SGC+FDR-Lite` 均已完成筛选，但没有满足“组合稳定优于 baseline 与任一单模块”的最终要求；`SGC / Strip-Guided Calibration` 单模块在 DIOR-R seed 42 上四项指标均优于 baseline 和单 FDF，可作为当前 C 候选。
