...
Baseline、A-P2、B-LSK、C-Dynamic、C-Dynamic-Plus、B-PKI-Lite、A+B-PKI-Lite 和 A+B-PKI-Lite+C-Plus 已完成 `test` split 评估。A-P2 相对 baseline 在全尺度和小目标指标上均有提升，可以作为有效消融基础；B-LSK 当前单独实验未提升；C-Dynamic、C-Dynamic-Plus 和 B-PKI-Lite 轻微正向；A+B-PKI-Lite 继续超过 A-P2，是当前最佳组合；A+B-PKI-Lite+C-Plus 明显高于 baseline，但低于 A+B-PKI-Lite，可作为三创新点融合消融记录。新版 C-GRA-Lite 已完成代码和配置，建议先训练单独 C，再根据结果训练 ABC-GRA-Lite。
...
| C-GRA-Lite | `experiments/dior/c_gra_lite_homews.yaml` | 待训练 | - | - | - | - | ready |
...
| A+B-PKI-Lite+C-GRA-Lite | `experiments/dior/abc_p2_pki_gra_lite_homews.yaml` | 待训练 | - | - | - | - | ready |
...
| C-GRA-Lite | 2,751,259 | +93,636 | +3.52% |
