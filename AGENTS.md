...
- 当前筛选结论：`C-v3 = SGC / Strip-Guided Calibration` 的 DIOR-R seed 42 单模块结果成立，四项指标均超过 LSKNet-T baseline 和单 FDF；但直接 `SGC+FDF` 组合四项均低于两个单模块，不继续扩展三 seed。`D = FDR-Lite / Frequency-Detail Residual Lite` 与 `SGC+FDR-Lite` 也已完成 seed 42 筛选：FDR-Lite 仅 All mAP50 略高，组合仍未超过 SGC，因此不作为定稿 C/D。尚无正式 C/D 定稿。
